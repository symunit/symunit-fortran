"""Release data for SymUnit.

When SymUnit is imported a number of steps are followed to determine
the version information.
   ...
   1) If the release is not a development release (dev=False), then version
      ...
      downloadable release of SymUnit since setup.py creates it during
      packaging/installation.  However, version.py might not exist if one
      is running SymUnit from the mercurial repository.  In the event that
...
SymUnit, we must make sure write_versionfile() does not overwrite the
...
def write_versionfile():
    ...
    text = '''"""
Version information for SymUnit, created during installation.
...
# Was SymUnit built from a development version? If so, remember that the major
...
DESCRIPTION = "Python package for automatic unit test generation"

LONG_DESCRIPTION = \
"""
SymUnit is a Python package for automatic unit test generation.

"""
LICENSE = 'T.B.D'
AUTHORS = {'Youngsung' : ('Youngsung Kim', 'grnydawn@gmail.com')}
MAINTAINER = "SymUnit Developers"
MAINTAINER_EMAIL = "symunit-discuss@googlegroups.com"
URL = 'http://symunit.github.io/'
DOWNLOAD_URL = 'T.B.D.'
PLATFORMS = ['Linux', 'Mac OSX', 'Windows', 'Unix']
KEYWORDS = ['Unit Test', 'Symbolic Computing', 'reStructuredText']
